...
1. **Verify task is in correct column:**
   ...
   - Architect watches: Analyse (Ready, Plan-Pending-Approval + Plan-Approved/Plan-Rejected)
   - Dev watches: Development (Planned, Rework-Requested)
   - Reviewer watches: Review (completion tags present, no Rework-Requested)
   - Ops watches: Review (Review-Approved + Ops-Ready), Deploy
...
4. **Review → Deploy:**
   ...
   - Add `Ops-Ready` tag (human merge approval)
