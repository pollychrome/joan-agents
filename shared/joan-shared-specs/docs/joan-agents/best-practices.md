...
After answering, add the `Clarification-Answered` tag so BA re-checks the task (BA clears `Needs-Clarification`).
...
- `Review-Approved` = Reviewer approved (still requires `Ops-Ready` to merge)
- `Ops-Ready` = Human approved merge, Ops may proceed
- `Rework-Requested` = Changes needed, back to Development
