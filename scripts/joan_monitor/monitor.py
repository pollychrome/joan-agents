...
class JoanMonitor:
    ...
    def _show_static_project_view(self, proj_name: str, info: dict):
        ...
        if startup.get("total_actionable", 0) > 0 or startup.get("dispatched", 0) > 0:
            ...
            startup_table = RichTable(show_header=False, box=None, padding=(0, 2))
            startup_table.add_column("Key", style="cyan")
            startup_table.add_column("Value")

            startup_table.add_row(
                "Actionable Tasks", str(startup.get("total_actionable", 0))
            )
            startup_table.add_row(
                "Handlers Dispatched", str(startup.get("dispatched", 0))
            )
            if startup.get("pipeline_blocked"):
                startup_table.add_row(
                    "Pipeline", f"[yellow]BLOCKED: {startup.get('pipeline_reason', '')}[/yellow]"
                )
            if startup.get("recovery_issues", 0) > 0:
                startup_table.add_row(
                    "Recovery Issues", f"[yellow]{startup['recovery_issues']}[/yellow]"
                )
            if startup.get("pending_human", 0) > 0:
                startup_table.add_row(
                    "Awaiting Human", f"[yellow]{startup['pending_human']}[/yellow]"
                )

            self.console.print(
                Panel(startup_table, title="\u26a1 Startup Dispatch", border_style="cyan")
            )
            self.console.print()
