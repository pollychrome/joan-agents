...
Implement features in isolated worktrees, enabling true parallel development.
...
Report: "Dev #{DEV_ID} initialized for project {PROJECT_NAME}"
...
IF TASK_QUEUE is empty:
  ...
  2. Find workable tasks (TWO categories):
     ...
     a. REWORK (PRIORITY) - Tasks needing revision:
     ...
     b. NEW WORK - Tasks with "Planned" tag:
        - Task is in "Development" column
  ...
  4. Handle empty queue:
     ...
     ELSE:
       ...
       Report: "Dev #{DEV_ID} found {queue.length} potential tasks"
```
...
is_rework = current_task has "Rework-Requested" OR "Merge-Conflict" tag
...
3. Continue to Phase 3 with rework context
```
...
5. Comment on task:
   ...
   ELSE:
     "Dev #{DEV_ID} started. Branch: {BRANCH}"
```
...
4. Update task (CRITICAL TAG OPERATIONS):
   - Remove: CLAIM_TAG
   - Remove: "Planned"
...
5. Comment on task (use canonical ## format for rework):
   IF is_merge_conflict:
...
6. Report: "Dev #{DEV_ID} completed '{title}'"
```
...
```
IF MODE == "single":
  After completing/failing ONE task, EXIT
...
- Task is in "Development" column
- Task has "Planned" OR "Rework-Requested" OR "Merge-Conflict" tag
- Task has NO "Claimed-Dev-*" tags
- Task has NO "Implementation-Failed" tag
